...
if not os.path.exists(MODEL_PATH):
    ...
    hist_windowed_df = hist_df_processed.groupBy(
        window(col("event_time"), "5 minutes", "1 minute")).agg(
    ...
    hist_features = hist_windowed_df.withColumn(
        "hour_of_day", hour(col("window.start"))
    ).withColumn(
        "minute_of_hour", minute(col("window.start"))
    ) 
    ...
    lr = LinearRegression(labelCol="avg_fare", featuresCol="features")
...
windowed_df = parsed_stream.groupBy(window(col("event_time"), "5 minutes", "1 minute")).agg(
        avg("fare_amount").alias("avg_fare")
)
# TODO: Apply the same feature engineering to the streaming windowed data.
# Create 'hour_of_day' and 'minute_of_hour' columns.
windowed_features = windowed_df.withColumn(
    "hour_of_day", hour(col("window.start"))
).withColumn(
    "minute_of_hour", minute(col("window.start"))
) 
